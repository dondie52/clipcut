...
import { TEXT_POSITION_PRESETS, TEXT_STYLE_PRESETS, DEFAULT_CLIP_PROPERTIES } from './constants';
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
          {TEXT_STYLE_PRESETS.map(preset => (
            ...
            >
              <Icon i={preset.icon} s={20} c="#75aadb" />
              <div>
                <div style={{ fontSize: '12px', fontWeight: 600, color: '#e2e8f0' }}>{preset.name}</div>
                <div style={{ fontSize: '9px', color: '#64748b', marginTop: '2px' }}>{preset.textSize}px</div>
              </div>
            </button>
          ))}
        </div>
