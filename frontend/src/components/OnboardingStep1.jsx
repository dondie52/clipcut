 ...
 *
 * @module components/OnboardingStep1
 * @description Onboarding Step 1 — Profile. "Warm Storybook" aesthetic:
 *   Outfit display + Spline Sans body, soft rounded geometry, huge serif
 *   background numeral, paint-stroke progress indicator. State, handlers,
 *   and sanitizeTextInput pipeline preserved.
 */
...
const OnboardingStep1 = ({ onContinue, onSkip, onSkipAll }) => {
  ...
  return (
    <div className="ob1-root">
      <style>{STEP1_CSS}</style>

      <span className="ob1-bg-num" aria-hidden="true">01</span>

      <header className="ob1-header">
        <div className="ob1-brand">
          <span className="ob1-brand-mono" aria-hidden="true">CC</span>
          <span className="ob1-brand-name">ClipCut</span>
        </div>
        <div className="ob1-progress">
          <div className="ob1-progress-strokes" aria-hidden="true">
            <div className="ob1-stroke is-active" />
            <div className="ob1-stroke" />
            <div className="ob1-stroke" />
          </div>
          <span className="ob1-step-label">Step 1 · of 3</span>
        </div>
      </header>

      <main className="ob1-main">
        <div className="ob1-container">
          <div className="ob1-heading">
            <div className="ob1-eyebrow">Your profile</div>
            <h1 className="ob1-title">
              Who's behind<br />
              the <span className="ob1-title-em">cut?</span>
            </h1>
            <p className="ob1-subtitle">
              Set up your creator identity. You can change any of this later in Settings.
            </p>
          </div>

          <div className="ob1-avatar-section">
            <div
              className={`ob1-avatar-btn ${avatarPreview ? "has-image" : ""}`}
              role="button"
              tabIndex={0}
            ...
            >
              {!avatarPreview && (
                <div className="ob1-avatar-placeholder">
                  <span className="material-symbols-outlined">add_a_photo</span>
                  <span className="ob1-avatar-placeholder-text">Upload</span>
                </div>
              )}
              <div className="ob1-avatar-edit" aria-hidden="true">
          ...
          <form className="ob1-form" onSubmit={(e) => e.preventDefault()}>
            <div className="ob1-field">
              <label htmlFor="ob1-name" className="ob1-label">Display Name</label>
              <input
                id="ob1-name"
                className="ob1-input"
                type="text"
                ...
                value={displayName}
                onChange={(e) => setDisplayName(sanitizeTextInput(e.target.value, { maxLength: 100 }))}
                autoComplete="name"
              />
            </div>

            <div className="ob1-field">
              <label htmlFor="ob1-bio" className="ob1-label">Bio</label>
              <textarea
                id="ob1-bio"
                className="ob1-input ob1-textarea"
                placeholder="Tell us about your editing style…"
                value={bio}
                onChange={(e) => setBio(sanitizeTextInput(e.target.value, { maxLength: 500, allowNewlines: true }))}
                rows={3}
              />
              <div className="ob1-char-count">{bio.length}/500</div>
            </div>

            <div className="ob1-actions">
              <button
                type="submit"
                className="ob1-btn-primary"
                onClick={() => { trackEvent(analyticsEvents.onboardingContinue, { step: "1" }); onContinue?.(); }}
              >
                Continue
              ...
              <button
                type="button"
                className="ob1-btn-skip"
                onClick={() => { trackEvent(analyticsEvents.onboardingSkip, { step: "1" }); onSkip?.(); }}
              >
                Skip this step
              </button>
              <button
                type="button"
                className="ob1-btn-skip-all"
                onClick={() => { trackEvent(analyticsEvents.onboardingSkip, { step: "1", action: "skip_all" }); onSkipAll?.(); }}
              >
