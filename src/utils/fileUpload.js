...
/**
 * Get upload progress for a file (for resume UI)
 * @param {File} file - File to check
 * @param {string} path - Storage path
 * @returns {number} Progress percentage (0-100), or 0 if not found
 */
export function getUploadProgress(file, path) {
  const fileId = generateFileId(file);
...
/**
 * Cancel and clear an in-progress upload
 * @param {File} file - File being uploaded
 */
export function cancelUpload(file) {
  const fileId = generateFileId(file);
  clearUploadState(fileId);
}
 ...
 */
export function clearAllUploadStates() {
  const keys = Object.keys(localStorage);
  for (const key of keys) {
    if (key.startsWith(UPLOAD_STATE_KEY)) {
